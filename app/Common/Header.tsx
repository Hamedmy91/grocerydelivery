...
const Header = () => {
  ...
  return (
    <header className="relative z-50">
      <div className="flex justify-between items-center gap-4 p-4 max-w-6xl mx-auto">
        ...
        <div className="hidden sm:flex flex-1 justify-center items-center gap-8">
          <ul className="flex gap-6 items-center text-header-res text-[14px] font-medium">
            <Link
              href="/"
              className="cursor-pointer hover:text-header transition-colors"
            >
              Home
            </Link>
            ...
            <Link href="/Deals" className="cursor-pointer text-header-res-1">
              Deals
            </Link>
          </ul>
          ...
          <div className="relative flex items-center">
            <input
              ...
              className="border border-header-res-1 bg-[#f7f6f4] w-[384px] h-9 rounded-full pl-9 pr-4 text-[14px] text-header outline-none"
            />

            <button
              onClick={handleSearch}
              aria-label="Search"
              className="absolute left-3 text-[18px] text-header"
            >
              <CiSearch />
            </button>
          </div>
        ...
        <div className="hidden sm:flex items-center gap-4 shrink-0">
          ...
          <Link
            href="/Sign"
            className="flex gap-2 items-center h-9 px-4 rounded-full bg-header text-white cursor-pointer hover:opacity-90 transition-opacity"
          >
      ...
      <div
        className={`sm:hidden absolute top-16 right-4 w-68 bg-white rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.12)] border border-[#eeeeee] overflow-hidden transition-all duration-200 origin-top-right ${
          menu
